...
print("\n⚠️ Colunas com possíveis tipos inconsistentes (ex: datas como string):", tipos_inconsistentes)
        ...
        if df[coluna].dtype == 'object':
            ...
            if amostra.str.contains(r'\d{1,2}/\d{1,2}/\d{4} \d{1,2}:\d{2}:\d{2} (AM|PM)', regex=True).any():
                try:
                    ...
                    print(f"📅 Data convertida: {nome}.{coluna}")
                except Exception as e:
                    print(f"⚠️ Erro ao converter data em {nome}.{coluna}: {e}")
...
if df[coluna].dtype == 'object':
            try:
                ...
                if is_numeric_dtype(df[coluna]):
                    print(f"🔢 Coluna convertida para número: {nome}.{coluna}")
            except Exception as e:
                print(f"⚠️ Erro ao converter número em {nome}.{coluna}: {e}")
...
cursor.execute("INSERT INTO produtos (nome, categoria, preço) VALUES (?, ?, ?)", ("Produto X", "Eletrônicos", 550.00))
cursor.execute("INSERT INTO produtos (nome, categoria, preço) VALUES (?, ?, ?)", ("Produto Y", "Livros", 45.00))
cursor.execute("INSERT INTO produtos (nome, categoria, preço) VALUES (?, ?, ?)", ("Produto Z", "Eletrônicos", 1200.00))
...
SELECT
    ...
    v.quantidade,
    p.preço,
    (v.quantidade * p.preço) AS valor_total_item,
    -- Usamos uma função de janela para numerar as compras de cada cliente+produto
    -- Ordenando pela data em ordem decrescente. A compra mais recente terá rank = 1.
...
-- Para obter apenas a última compra, precisaremos filtrar por rn = 1 ao consultar a VIEW.
;
""")
conn.commit()
...
FROM (
    -- Esta é a subquery: calcula o ticket médio para cada cliente
    SELECT
        c.nome,
        SUM(p.preço * v.quantidade) * 1.0 / COUNT(v.id_venda) AS ticket_medio
    FROM vendas v
    JOIN clientes c ON v.id_cliente = c.id_cliente
    JOIN produtos p ON v.id_produto = p.id_produto
    GROUP BY c.nome
) AS sub -- 'sub' é o alias dado ao resultado da subquery, tratando-o como uma tabela
WHERE sub.ticket_medio > 300 -- O filtro é aplicado no resultado da subquery
...
data = {
    ...
    'preço': [550.0, 550.0, 45.0, 1200.0, 30.0, 45.0, 30.0, 150.0],
...
'''Última Compra por Cliente e Produto
Abaixo estão listadas as compras mais recentes feitas por cada cliente para cada produto, considerando apenas a compra mais atual (com rn = 1):

Cliente	Produto	Data da Compra	Quantidade	Preço Unitário	Valor Total
...
💰 Ticket Médio por Cliente
O ticket médio representa o valor médio gasto por compra recente, considerando apenas a última compra de cada produto por cliente:
...
O Cliente A possui o maior ticket médio, impulsionado principalmente pela compra de dois itens do Produto X com valor elevado.

O Cliente B também apresenta um ticket médio alto, com destaque para a aquisição do Produto Z de alto valor unitário.

O Cliente C apresenta o menor ticket médio, concentrando suas compras em produtos de baixo custo e menor quantidade.

📌 Conclusões e Insights
Segmentação de clientes por ticket médio pode ajudar a traçar estratégias personalizadas de marketing:

Clientes A e B podem ser classificados como clientes de alto valor, com potencial para campanhas de retenção e fidelização.

Cliente C é um perfil mais econômico, ideal para campanhas de incentivo à recorrência ou combos promocionais.

Produtos de maior valor (como Produto Z e Produto X) influenciam diretamente o ticket médio e devem ser acompanhados de estratégias que reforcem seu valor percebido.

Monitorar a frequência de compra por produto pode abrir oportunidades para programas de fidelidade ou vendas cruzadas.
'''
